...
model_config_list = [{"model": "deepseek-chat",
...
ragUserProxyAgent = RetrieveUserProxyAgent(
    name="interviewee",
    human_input_mode="ALWAYS",
    system_message="",
    max_consecutive_auto_reply=0,
    retrieve_config=retrieve_config_list[0],
    code_execution_config=False,  # set to False if you don't want to execute the code
    description="Interviewee who want to conduct a mock interview."
)

questionsSelectorAssistantAgent = AssistantAgent(
...
interviewerAssistantAgent = AssistantAgent(
    name='interviewer',
...
finalSummarizeMakerAssistantAgent = AssistantAgent(
...
def reset_agents():
    questionsSelectorAssistantAgent.reset()
    interviewerAssistantAgent.reset()
    finalSummarizeMakerAssistantAgent.reset()


def next_speaker_selection_func(last_speaker: Agent, simulateGroupChat: GroupChat):
    messages = simulateGroupChat.messages

    if len(messages) < 1:
        return ragUserProxyAgent

    elif len(messages) == 1:
        return questionsSelectorAssistantAgent

    elif last_speaker is questionsSelectorAssistantAgent:
        return interviewerAssistantAgent

    elif last_speaker is interviewerAssistantAgent:
        if "请你回答" in messages[-1]["content"]:
            return ragUserProxyAgent
        else:
            return finalSummarizeMakerAssistantAgent

    elif last_speaker is ragUserProxyAgent:
        return interviewerAssistantAgent

    elif last_speaker is finalSummarizeMakerAssistantAgent:
        return ragUserProxyAgent
...
def initial_chat():
    reset_agents()
    simulateGroupChat = GroupChat(
        agents=[ragUserProxyAgent, questionsSelectorAssistantAgent, interviewerAssistantAgent,
                finalSummarizeMakerAssistantAgent],
        messages=[],
        max_round=20,
        speaker_selection_method=next_speaker_selection_func,
        send_introductions=True
    )
    manager = GroupChatManager(groupchat=simulateGroupChat, llm_config={"config_list": model_config_list})

    ragUserProxyAgent.initiate_chat(
        manager,
        message=ragUserProxyAgent.message_generator,
